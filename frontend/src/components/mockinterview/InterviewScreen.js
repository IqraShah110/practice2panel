...
const InterviewScreen = ({ sessionData, onEndInterview, onRestart, apiBaseUrl }) => {
  ...
  const statusText = useMemo(() => {
    ...
    if (loading) return 'Processing response…';
    return '';
  }, [listening, loading]);

  // Speak welcome message and first question when interview starts (behavioral only)
  useEffect(() => {
    if (voiceMode && sessionData.welcome_message && sessionData.interview_type?.toLowerCase() === "behavioral" && questionNumber === 1 && currentQuestion) {
      speak(sessionData.welcome_message).then(() => {
  ...
  // Speak question automatically in voice mode when it changes (but not the first one)
  useEffect(() => {
    if (voiceMode && currentQuestion && questionNumber > 1) {
      const intro = isFollowUp ? 'Follow up question.' : `Question ${questionNumber}.`;
      speak(`${intro} ${currentQuestion}`);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [currentQuestion, voiceMode, isFollowUp, questionNumber]);
  ...
  const handleInteract = async (userInput) => {
    ...
    try {
      const response = await axios.post(`${baseUrl}/api/mock-interview/interact`, {
      ...
      if (intent === 'repeat_question') {
        ...
        if (response.data.completed) {
          setIsCompleted(true);
          const endResponse = await axios.post(`${baseUrl}/api/mock-interview/end-interview`, {
            session_id: sessionData.session_id
          });
  ...
  const handleEndInterview = async () => {
    ...
    const baseUrl = apiBaseUrl || API_BASE_URL;
    try {
      const response = await axios.post(`${baseUrl}/api/mock-interview/end-interview`, {
        session_id: sessionData.session_id
      });
  ...
  return (
    <div>
      ...
      <div className="session-header">
        ...
        <div className="timer"><span role="img" aria-label="timer">⏲️</span>&nbsp; {formattedTime}</div>
      </div>
        ...
        <div className="button-group">
          <button 
            ...
            onClick={async () => {
              ...
              try {
                setLoading(true);
                const res = await axios.post(`${baseUrl}/api/mock-interview/next-question`, { session_id: sessionData.session_id });
                ...
                if (nextQ) {
                  ...
                  setIsCompleted(true);
                  const endResponse = await axios.post(`${baseUrl}/api/mock-interview/end-interview`, { session_id: sessionData.session_id });
                  onEndInterview(endResponse.data);
                }
              } catch (e) {
